  ...
  useEffect(() => {
    ...
    return (
     <section
             id="projects"
             ref={projectsRef}
             className="relative min-h-screen flex flex-col justify-center items-center text-white py-40 pt-20"
           >
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[250px]">
                 ...
                 {projects.map((project, index) => (
                   ...
                   >
                     ...
                     {/* Hover Overlay */}
                     <div className="absolute inset-0 bg-black bg-opacity-70 flex flex-col justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-300 p-4">
                       {/* Project Title */}
                       <h3 className="text-xl font-bold text-[#00E0FF]">
                         {project.title}
                       </h3>
                       <p className="text-xl font text-[#00E0FF]">
                         {project.description}
                       </p>
                       ...
                       <div className="flex flex-wrap gap-2 justify-center">
                         {project.technologies.map((tech, idx) => (
                           <span
                             key={idx}
                             className="bg-[#00E0FF] text-black px-2 py-1 rounded-lg text-sm font-semibold "
                           >
                             {tech}
                           </span>
                         ))}
